...
vector<PIRQuery> BatchPIRClient::create_queries(vector<uint64_t> batch)
{
    ...
    {
        auto query = client_list_[i].gen_query(batch);
...
bool BatchPIRClient::cuckoo_hash(vector<uint64_t> batch)
{
    ...
    auto num_candidates = tree_height_;

    if (batch.size() != tree_height_)
    {
        cout << batch.size() << " " << tree_height_ << " " << endl;
        throw std::invalid_argument("Error: Batch size is wrong");
    }

    cuckoo_table_.resize(tree_height_, batchpir_params_.get_default_value());
...
void BatchPIRClient::translate_cuckoo()
{
    ...
    {
        //cout << "cuckoo_table_ Previous (index) = " << cuckoo_table_[i] << endl;
        // check if bucket is empty
        if (cuckoo_table_[i] != batchpir_params_.get_default_value())
        {
            //cout << "candidates = " << i << endl;
            //cout << "map key = " << to_string(cuckoo_table_[i]) + " " + to_string(i) << endl;
            // convert from db index to bucket index
            cuckoo_table_[i] = map_[to_string(cuckoo_table_[i]) + "_" + to_string(i)];
            //cout << "cuckoo_table_ After (map value) = " << cuckoo_table_[i] << endl;
        }
    }
}
