...
void testHighRate(size_t num_expansions, size_t further_dims, size_t idx_target) {
    ...
    for (size_t trial = 0; trial < trials; trial++) {
        ...
        // MARK: Processing the first dimension.
        ...
        // MARK: Fold in subsequent dimensions.
    ...
    // MARK: Perform the modulus switching on the first row.
    size_t qprime_bits = bits_to_hold_arb_qprime;
    uint64_t arb_qprime_h = qprime_mods[qprime_bits];

    uint64_t q_1 = 4*p_db;

    // MARK: Rescale the final encoding to obtain compactness.
    ...
    // MARK: Recover to get the matrix encoding Z.
    ...
    // MARK: Decode to get the encoded message.
    ...
    MatPoly Z_uncrtd = s_prod;//from_ntt(Z);  // Redundant encoding = M.
    // divide_by_const(Z_uncrtd, Z_uncrtd, p_db, Q_i, p_db);
    time_decoding += et();

    // MatPoly top_left = pick(Z_uncrtd, 0, 0, 1, 1);
    // MARK: Calculate the correct result.
